...
gulp.task('less-custom', function() {
	return 	gulp.src(parentFolder + 'less/custom.less').
			pipe(less()).
			pipe(minifyCSS({
				mediaMerging: true
			})).
...
gulp.task('watch', function () {
	...
	gulp.watch(parentFolder + 'less/**/*.less', ['less-style','less-custom']);
...
gulp.task('dev', ['less-style','less-custom', 'scripts', 'watch']);

// Production Build [run 'gulp prod' in your terminal]
gulp.task('prod', ['less-style','less-custom', 'scriptsMin']);
